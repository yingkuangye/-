...
#define SLAVE_ADDRESS 0x90 //定义从机写地址（写入tmp275）
#define POINT_REGISTER_TEMPERATURE 0x00 //定义point register的地址，指向温度寄存器
#define POINT_REGISTER_CONFIGURATION 0x01 //定义point register的地址，指向初始化寄存器
#define POINT_REGISTER_TLOW 0x02//定义point register的地址，指向TLOW寄存器
#define POINT_REGISTER_THIGH 0x03//定义point register的地址，指向THIGH寄存器



/*等待超时*/
#define I2CT_FLAG_TIMEOUT         ((uint32_t)0x00FF)
